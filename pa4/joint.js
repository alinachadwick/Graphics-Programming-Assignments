...
Joint.prototype.getLocalMatrix = function() { //probably correct, if bug then its on line 50
	var rotateM = Matrix.rotate(this.mJointAngle, this.mJointAxis.x, this.mJointAxis.y, this.mJointAxis.z);
	var translateM = Matrix.translate(this.mPosition.x, this.mPosition.y, this.mPosition.z, localM);
	var localM = Matrix.multiply(translateM, rotateM);
	return localM;
}
...
Joint.prototype.getWorldMatrix = function() {  
		var localM = this.getLocalMatrix();
		if (this.mParent != null) {
			var pmatrix = this.mParent.getLocalMatrix(localM)
			worldM = Matrix.multiply(pmatrix, localM);
		}
		else {
			worldM = localM;
		}
...
// ################
	  var getWorld = this.getWorldMatrix();
    this.mBindingMatrix = Matrix.inverse(getWorld);
