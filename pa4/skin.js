...
function computeDistanceToLine(pt, vertex0, vertex1)
{
	...
	var dot_product = proj1.dot(proj2);
	var squared_length = proj2.x * proj2.x + proj2.y *proj2.y +proj2.z *proj2.z

	var location = dot_product / squared_length;

	var close = new Vector();

	//case 1 lies outside but closer vertex1
	if (location >= 1) {
		close = vertex1;
	}

	//case 2 lies outside but closer vertex0
	else if (location <= 0){
		close = vertex0;
	}
	//case 3 lies inside line segment
	else {
		close = vertex0.add(proj2.multiply(location));
	}

	var x_dist = (pt.x - close.x)**2;
	var y_dist = (pt.y - close.y)**2;
	var z_dist = (pt.z - close.z)**2;
...
SkinMesh.prototype.rigidSkinning = function()
{
    ...
    if(this.mSkeleton){
		for (var vertex = 0; vertex < this.getNumVertices(); vertex++){
			var rigid = this.getRigidlyAttachedJoint(vertex); //a joint
			var local = rigid.getBindingMatrix(); 
			var wor = rigid.getWorldMatrix();  
			var x = this.getVertex(vertex);
			var bone = local.transformPoint(x);
			var transformed = wor.transformPoint(bone);
			this.setTransformedVertex(vertex, transformed);
		}
	}
	else
	{
...
SkinMesh.prototype.computeLinearBlendedWeights = function()
{
    ...
    if(this.mSkeleton) {
		...
		for (var vertex = 0; vertex < this.getNumVertices(); vertex++){
			var point = this.getVertex(vertex);
			
			for (var joint = 0; joint < this.mSkeleton.getNumJoints(); joint++){
				var j = this.mSkeleton.getJoint(joint);
				var worldSpacePosition = j.getJointEndPoints();
				var dist = computeDistanceToLine(point, worldSpacePosition.v1, worldSpacePosition.v0);
				weight = 1 / (dist**4);
				this.setVertexWeight(vertex, joint, weight);
			}
			var composite_weight = 0;
			for (var joint = 0; joint < this.mSkeleton.getNumJoints(); joint++){
				var temp_w = this.getVertexWeight(vertex, joint);
				composite_weight = composite_weight+temp_w;
			}

			for (var joint = 0; joint < this.mSkeleton.getNumJoints(); joint++) {
				var temp_w = this.getVertexWeight(vertex, joint);
				var curr_weight = temp_w/composite_weight;
				this.setVertexWeight(vertex, joint, curr_weight); 
			}			
		}
	}
	else
...
SkinMesh.prototype.linearBlendSkinning = function()
{
	...
	if (this.mSkeleton) {
		...
		for (var vertex = 0; vertex < this.getNumVertices(); vertex++){
			var currentPos = this.getVertex(vertex);
			var transformedPosition = new Vector();
			for (var joint = 0; joint < this.mSkeleton.getNumJoints(); joint++){
				var wt = this.getVertexWeight(vertex, joint);

				var local = this.mSkeleton.getJoint(joint); 
				
		  	var wor = local.getWorldMatrix();  
				
			  var bone = local.getBindingMatrix();
				var bonetransformed = bone.transformPoint(currentPos);
			  var worldtransformed = wor.transformPoint(bonetransformed);
				var last = worldtransformed.multiply(wt);
				transformedPosition.x = transformedPosition.x + last.x;
				transformedPosition.y = transformedPosition.y + last.y;
				transformedPosition.z = transformedPosition.z + last.z;
			}
			this.setTransformedVertex(vertex, transformedPosition);
